...
float wallPos = 130.0;
float wallThick = 10.0;
int nBallRadius = 10;
float holeRadius = 20.0;
int nCirleCenter[2][2];

float stiffnessK = 150.0;
float dampingB = 0.0;
float maxForce = 4.5;
bool useConstraints = true;

float zeroOffset[3] = {110.0, 112.0, 112.0};
bool calibrateRequested = false;

bool serialTrigger = false;
const byte PKT_H1 = 0xAA;
const byte PKT_H2 = 0x55;
byte pktState = 0;
byte pktLen = 0;
byte pktIndex = 0;
byte pktBuf[32];

#define UNITY_PACKET 0
const byte POS_H1 = 0xFE;
const byte POS_H2 = 0xEF;
const byte FORCE_H1 = 0xCC;
const byte FORCE_H2 = 0x33;
...
bool bIntersectCirle(double* b)
{
    ...
    for(int i = 0; i < 3; i++)
    {
     ...
     double dCCCC =  (holeRadius+nBallRadius)*(holeRadius+nBallRadius);
     if(distace < dCCCC) 
       {
           return true;
       }
...
{   
  ...
  {
    zeroOffset[0] = ts;
    zeroOffset[1] = handle_angle[1];
    zeroOffset[2] = handle_angle[2];
    calibrateRequested = false;
  }

     double RealAngle[3]; 
     RealAngle[0] = zeroOffset[0]-ts; 
     RealAngle[1] = zeroOffset[1]-handle_angle[1];
     RealAngle[2] = zeroOffset[2]-handle_angle[2];
    ...
    xh = -((-b[2]-nBallRadius)-wallPos-wallThick/2)/1000.0;  

    static unsigned long lastMicros = 0;
    unsigned long nowMicros = micros();
    double dt = (lastMicros == 0) ? 0.02 : (nowMicros - lastMicros) / 1000000.0;
    if (dt <= 0.0) dt = 0.02;
    lastMicros = nowMicros;

    dxh = (xh - xh_prev) / dt;
    dxh_filt = 0.9 * dxh_filt + 0.1 * dxh;
    xh_prev = xh;
   
    bool bInverst = false;
    if(xh > (nBallRadius+wallThick/2)/1000.0) bInverst = true;  

    if(!useConstraints) {
      force = 0;
    }
    else if(xh < 0.0 || xh > 0.002*(nBallRadius+wallThick/2)) force = 0;
    ...
    { 
        ...
        else
        {
           if (bInverst) // virtual wall force feedback rendering
            {              
              force = stiffnessK * fabs((2*nBallRadius+wallThick)/1000.0-xh);
            }
    ...
    force -= dampingB * dxh_filt;
    if(force > maxForce) force = maxForce;  // max force
    ...
    ////////////////////   
          ...
          #if UNITY_PACKET
            sendPositionPacket(handle_pos, sign);
            sendForcePacket((float)force);
          #else
...
void parseSerialByte(byte b) {
  ...
  if (pktState == 1) {
    ...
    pktState = 0;
    if (b == 'A') serialTrigger = true;
    if (b == 'C') { serialTrigger = true; calibrateRequested = true; }
    return;
  }
  ...
  if (pktState == 3) {
    pktBuf[pktIndex++] = b;
    if (pktIndex >= pktLen) {
      applyParams(pktBuf, pktLen);
      pktState = 0;
    }
  }
}

float readFloatFromBuf(byte* buf, int index) {
...
void applyParams(byte* buf, int len) {
  if (len < 25) return;
  byte flags = buf[0];
  useConstraints = (flags & 0x01) != 0;

  wallPos = readFloatFromBuf(buf, 1);
  wallThick = readFloatFromBuf(buf, 5);
  holeRadius = readFloatFromBuf(buf, 9);
  stiffnessK = readFloatFromBuf(buf, 13);
  dampingB = readFloatFromBuf(buf, 17);
  maxForce = readFloatFromBuf(buf, 21);
}

void sendPositionPacket(int* pos, byte* sign) {
  Serial.write(POS_H1);
  Serial.write(POS_H2);
  Serial.write((byte)14);
  Serial.write(pos[0]&0xFF);  Serial.write((pos[0]>>8)&0xFF); Serial.write((pos[0]>>16)&0xFF); Serial.write((pos[0]>>24)&0xFF); 
  Serial.write(pos[1]&0xFF);  Serial.write((pos[1]>>8)&0xFF); Serial.write((pos[1]>>16)&0xFF); Serial.write((pos[1]>>24)&0xFF); 
  Serial.write(pos[2]&0xFF);  Serial.write((pos[2]>>8)&0xFF); Serial.write((pos[2]>>16)&0xFF); Serial.write((pos[2]>>24)&0xFF); 
  Serial.write(sign[0]); Serial.write(sign[1]);
}

void sendForcePacket(float f) {
  union { byte b[4]; float f; } u;
  u.f = f;
  Serial.write(FORCE_H1);
  Serial.write(FORCE_H2);
  Serial.write((byte)4);
  Serial.write(u.b[0]);
  Serial.write(u.b[1]);
  Serial.write(u.b[2]);
